...
if (!$EmailQueue = $modx->getService('emailqueue', 'EmailQueue', $modx->getOption('emailqueue_core_path', null,
        $modx->getOption('core_path') . 'components/emailqueue/') . 'model/emailqueue/', $scriptProperties)
) {
    return 'Could not load EmailQueue class!';
}
...
$c->limit($limit);
$items = $modx->getIterator('EmailQueueItem', $c);
